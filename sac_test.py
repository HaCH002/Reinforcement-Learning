...
def play_one_episode(env, agent):
    temp_replay_buffer = ReplayBuffer(mem_size=2000 + 50, state_dim=agent.state_dims,
...
env = gym.wrappers.RecordVideo(env, "./video", episode_trigger=lambda ep_id: (ep_id % 25 == 0), video_length=0)
#env = gym.wrappers.Monitor(env, "./video", video_callable=lambda ep_id: (ep_id >= 3000 and ep_id%25 == 0), force=True)
...
if training:
    ...
    for t in range(n_iterations):
        ...
        if t % 1 == 0:
            log_f.write(f'episode: {t+1}, reward: {total_reward}\n')
            log_f.flush()
        ...
        if t % 20 == 0:
            plot_data.append([t, np.array(reward_list).mean(), np.array(reward_list).std()])
            reward_list = []
