...
class CriticNetwork(nn.Module):
    ...
    def forward(self, state, action, hist_observation, hist_action, hist_length):
        ...
        last_time_steps = T.from_numpy(last_time_steps).to(self.device)
...
class ActorNetwork(nn.Module):
    def __init__(self, state_dims, action_dims, action_max, fc1_dims=128, lstm_hidden_size=128,
        ...
        self.action_max = T.from_numpy(action_max).to(self.device)
...
class Agent:
    ...
    def update(self, train_actor):
        ...
        states = T.from_numpy(states).to(T.float32).to(self.actor.device)
        actions = T.from_numpy(actions).to(T.float32).to(self.actor.device)
        rewards = T.from_numpy(rewards).to(T.float32).to(self.actor.device)
        new_states = T.from_numpy(new_states).to(T.float32).to(self.actor.device)
        dones = T.from_numpy(dones).to(self.actor.device)
        hist_state = T.from_numpy(hist_state).to(T.float32).to(self.actor.device)
        hist_action = T.from_numpy(hist_action).to(T.float32).to(self.actor.device)
        hist_next_state = T.from_numpy(hist_next_state).to(T.float32).to(self.actor.device)
        hist_next_action = T.from_numpy(hist_next_action).to(T.float32).to(self.actor.device)
